...
class EmbeddingManager:
    ...
    def initialize_embeddings(self):
        ...
        try:
            if self.config.EMBEDDING_MODEL_TYPE == "ollama":
                ...
                logger.info(
                    f"Initialized Ollama embeddings: {self.config.EMBEDDING_MODEL_NAME}"
                )

            elif self.config.EMBEDDING_MODEL_TYPE == "huggingface":
                self.embeddings = self._setup_huggingface_embeddings()
                logger.info(
                    f"Initialized HuggingFace embeddings: {self.config.EMBEDDING_MODEL_NAME}"
                )

            else:
                raise ValueError(
                    f"Unsupported embedding type: {self.config.EMBEDDING_MODEL_TYPE}"
                )
    ...
    def _setup_huggingface_embeddings(self):
        ...
        model_name = self.config.VIETNAMESE_EMBEDDING_MODELS.get(
            self.config.EMBEDDING_MODEL_NAME, self.config.EMBEDDING_MODEL_NAME
        )
    ...
    def get_available_models(self):
        ...
        return {"ollama": ollama_models, "huggingface": hf_models}
