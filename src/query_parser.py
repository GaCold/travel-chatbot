...
class QueryParser:
    ...
    def __init__(self):
        ...
        self.type_keywords_map = {
            ...
            "attraction": ["checkin", "tham quan", "chơi", "địa điểm du lịch", "cảnh đẹp", "đi đâu", "chỗ nào đẹp", "khám phá"], # Gộp "khám phá" vào đây
            "accommodation": ["lưu trú", "ở đâu", "khách sạn", "nhà nghỉ", "resort", "chỗ ở", "homestay"],
            "transport": ["di chuyển", "đi lại", "phương tiện", "bắt xe", "đến đó thế nào", "đi bằng gì", "tàu", "xe khách", "máy bay"],
        ...
        self.location_map = {
            ...
            "khanh_hoa_4759133": (["khánh hòa", "khánh hoà", "cam ranh"], "location_city"),
    ...
    def parse_query(self, question: str) -> Dict[str, any]:
        ...
        detected_region = self._detect_from_map(question_lower, self.region_keywords_map)
        if detected_region:
        ...
        detected_loc_specific = self._detect_location(question_lower, "location_specific")
    ...
    def _detect_from_map(self, text: str, keyword_map: Dict[str, List[str]]) -> Optional[str]:
    ...
    def _detect_location(self, text: str, loc_type: str) -> Optional[str]:
        ...
        if matches:
            matches.sort(key=lambda x: x[1], reverse=True)
            return matches[0][0] # Trả về map_key chuẩn (ví dụ: "da_nang", "ba_na_hills")
        return None
    ...
    def build_filter_dict(self, parsed_query: Dict) -> Optional[Dict]:
        ...
        if parsed_query.get("filters"):
            ...
            if parsed_query["filters"].get("location_specific"):
                filters.append({"location_specific": {"$eq": parsed_query["filters"]["location_specific"]}})
            # Nếu không có specific, mới lọc "location_city"
            elif parsed_query["filters"].get("location_city"):
                filters.append({"location_city": {"$eq": parsed_query["filters"]["location_city"]}})
